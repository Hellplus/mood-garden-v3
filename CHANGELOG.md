...
- 优化数据页备份与导出说明，明确 TXT 是可阅读日记，JSON 是完整数据备份，适合恢复或迁移。
- 优化恢复数据区文案，明确合并导入会追加记录且不会清空已有记录，覆盖导入会替换当前记录且覆盖后无法撤销。
- 优化覆盖导入确认 Dialog、导入导出 Toast 和首次引导中的本地保存 / JSON 备份提醒。
- 补充 README 中的本地保存、TXT / JSON 备份区别、导入风险和 PWA 不同步说明。
- 本阶段只调整文案和提示体验，不新增业务功能，不改变 records localStorage 主结构，也不修改 JSON 导入导出结构或 CRUD、筛选、搜索、排序、日历、分析逻辑。

## V3.1-DX.6.1

- 专项补强桌面端空状态插画尺寸约束，覆盖回顾 / 日历 / 今日无记录与“去记录”CTA 场景，避免插画按原始尺寸撑高卡片。
- 热修桌面端布局观感，收紧“去记录”空状态 / 引导区域中的插画尺寸和空状态卡片高度。
- 将桌面端标签云调整为更紧凑的 chip wrap，降低 tag chip 高度、图标尺寸和空白占比，不改变标签筛选逻辑。
- 将桌面端主题外观改为紧凑网格卡片，控制主题卡高度和图标尺寸，不改变主题切换逻辑。
- 本阶段只调整桌面展示层和文档记录，不新增业务功能，不改变 records localStorage 主结构，也不修改 CRUD、筛选、搜索、排序、日历、分析或导入导出核心逻辑。
