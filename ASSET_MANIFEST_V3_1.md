...
- 可以包含 PNG 或 SVG，现阶段以已经补齐并登记的正式素材为准；
- 命名必须清楚、稳定、可维护。
...
这些图片不是普通参考图，而是页面元素蓝图和后续 UI 实现、验收标准。实现不要求 1:1 像素复刻，但蓝图中明确存在的必要元素、结构层级、背景装饰、入口卡、空状态、弹窗、确认 Dialog 和页面级顶部装饰必须具备。Codex 不得脱离这些图自行重新设计图标、空状态、花朵或装饰元素。
...
6. 页面背景不依赖第五批 `patterns/` 纹理素材；后续 Codex 不得引用 `src/assets/ui/patterns/`，背景层优先使用已有 `backgrounds/`、`decorations/`、`paper/` 素材，以及 CSS 的柔和渐变和色块。

## 12. 必须使用正式素材的视觉元素

以下视觉元素必须优先使用 `src/assets/ui/` 中已经登记的正式 PNG / SVG 素材：
...
- 正式素材已经按 01-09 UI 图的视觉方向补齐；
- 正式素材能保证跨页面风格统一；
- 正式素材避免 emoji、浏览器默认符号或临时图形造成的系统差异；
- CSS 只负责布局、尺寸、状态、柔和渐变和必要色彩，不负责伪造正式素材。
...
如果第五批已经提供纸感、分割装饰、反馈状态或徽章类正式素材，后续实现必须优先使用正式素材；CSS 只能承担布局、基础容器、状态样式、柔和渐变和色块背景，不能用临时 CSS 图形伪造这些蓝图元素。页面背景不依赖第五批 `patterns/` 纹理素材，后续 Codex 不得引用 `src/assets/ui/patterns/`，背景层优先使用 `backgrounds/`、`decorations/`、`paper/` 和 CSS 柔和渐变。
...
emoji 可以在早期占位中出现，但 V3.1 UI 素材库落地后，正式 UI 应使用 `src/assets/ui/` 中登记的正式 PNG / SVG 素材，避免不同系统渲染不一致。CSS 只能负责布局、尺寸、渐变、状态和基础容器，不能用来伪造已有正式素材。
