    ...
    <section className="surface-panel data-panel">
      ...
      <section className="data-section data-section--export" aria-labelledby="export-title">
        <div className="data-section-heading">
          ...
          <p>可以导出可阅读的文字日记，也可以保存完整 JSON 备份；导出内容不受当前筛选影响。</p>
        </div>

        <div className="data-actions">
          <DataActionButton
            description="保存为可阅读的 TXT 日记，适合整理和留档；如果要恢复花园，请导出 JSON 备份。"
            icon={dataIcons.txtExport}
            onClick={onExportText}
            title="导出文字日记"
          />
          <DataActionButton
            description="完整保存当前花园数据，适合以后恢复或迁移到其他浏览器。"
      ...
      <section className="data-section data-section--restore" aria-labelledby="restore-title">
        <div className="data-section-heading">
          ...
          <p>请选择 Mood Garden 导出的 JSON 备份，再决定把它合并进来或覆盖当前记录。</p>
        </div>

        <div className="data-actions">
          <DataActionButton
            description={selectedImportFile ? `已选择：${selectedImportFile.name}` : '只能选择 Mood Garden 导出的 JSON 完整备份文件。'}
            icon={dataChooseFileImage}
            onClick={() => openFilePicker('select')}
            title="选择备份文件"
          />
          <DataActionButton
            description="把备份中的记录追加到当前花园，不会清空已有记录。"
          ...
          />
          <DataActionButton
            description="会用备份文件替换当前记录，覆盖后无法撤销；建议先导出当前 JSON 备份。"
        ...
        <div className="data-import-help">
          ...
          <span>
            ...
            <small>请确认它是 Mood Garden 导出的 JSON 备份，再重新选择。</small>
          </span>
        </div>
      </section>
      ...
      <section className="data-section data-section--local" aria-labelledby="local-data-title">
        <div className="data-section-heading">
          ...
          <p>记录只保存在当前浏览器里，换设备或换浏览器不会自动同步。</p>
        </div>

        <div className="data-note-grid" aria-label="数据保存说明">
          <DataNoteCard
            description="你的心情花只保存在这个浏览器，刷新页面后会从本机恢复。"
            icon={dataIcons.localStorage}
            title="记录只保存在当前浏览器"
          />
          <DataNoteCard
            description="更换手机、电脑或浏览器后，记录不会自己同步过去。"
            icon={dataDeviceNoSyncImage}
            title="换设备不会自动同步"
          />
          <DataNoteCard
            description="想长期保存或迁移记录，建议定期导出 JSON 备份。"
        ...
        <div className="empty-state backup-reminder-state">
          ...
          <p>记录只保存在当前浏览器，定期导出 JSON 备份会更安心。</p>
      ...
      <section className="data-section data-section--pwa" aria-labelledby="pwa-title">
        <div className="data-section-heading">
          ...
          <p>添加到主屏幕只是方便打开，不代表账号登录，也不会自动同步到其他设备。</p>
        </div>
        ...
        <div className="data-note-grid" aria-label="应用安装说明">
          <article>
            ...
            <p>添加到主屏幕后更方便打开，但记录仍只在当前浏览器保存，不会自动同步。</p>
            <button className="text-action data-guide-toggle" type="button" onClick={() => setIsPwaGuideOpen((value) => !value)}>
              查看教程 ›
            </button>
            {isPwaGuideOpen ? (
              <small className="data-guide-note">在浏览器菜单中选择“添加到主屏幕”或“安装应用”。这只是打开入口，不会创建账号或云同步。</small>
            ) : null}
          </article>
        </div>
      </section>
      ...
      <section className="data-section data-section--danger" aria-labelledby="danger-data-title">
        <div className="data-section-heading">
          ...
          <p>这些操作会影响当前花园，执行前会再给你一次确认。</p>
        </div>

        <div className="data-actions">
          {canClearRecords ? (
            <DataActionButton
              description={recordCount > 0 ? '删除后无法撤销，建议先导出 JSON 备份。' : '当前没有可删除的记录。'}
        ...
        <div className="data-danger-note">
          ...
          <span>{canClearRecords ? '删除前建议先导出当前 JSON 备份。' : '这里暂时没有可以执行的清空入口。'}</span>
        </div>
      </section>
      ...
      <ConfirmDialog
        cancelLabel="取消"
        confirmLabel="确认覆盖"
        description="这会用备份文件替换当前浏览器里的全部记录，覆盖后无法撤销。"
        image={dataCoverImportConfirmImage}
        isOpen={isReplaceConfirmOpen}
        note="建议先导出当前 JSON 备份，再继续覆盖导入。"
        title="确认覆盖导入？"
        variant="danger"
